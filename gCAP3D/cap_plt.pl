...
sub plot {
  ...
  ($nn,$height) = (12,10);	# 12-2=10 rows of traces per 10 in.
  ...
  $outps = "$mdl.ps";
  if ($am>0.) {$am = "$am/0";} else {$am=-$am;}
  $plt1 = "| pssac -JX$width/$height -R0/$tt/0/$nn -Y0.8 -Ent-2 -M$am -K >> $outps";
  $plts = "| psxy -JX -R -W2 -O -K >> $outps";
  $plt2 = "| pstext -JX -R -O -K -N >> $outps";
  $plt3 = "| psmeca -JX1/1 -R-1/1/-1/1 -Sa1 -G100 -Y9 -X-0.7 -O -K >> $outps";
  $plt3 = "| psmeca -JX1/1 -R-1/1/-1/1 -Sm1.08 -G100 -Y9 -X-0.7 -O -K >> $outps" if $tensor[1] eq "MomentTensor" and ($meca[18] ne "0.00" or $meca[21] ne "0.00");
  ...
  foreach (@rslt) {
    @aa = split;
    next if $aa[2] == 0;
    $x = `saclst az user1 f ${mdl}_$aa[0].? | head -1`;
    @aa = split(' ', $x);
  ...
  while (@rslt) {
    ...
    printf PLT "%f %f 12 0 0 TC $sec_per_inch s\n",0.5*$sec_per_inch,$y+0.3;
    $x = 0.5*$sec_per_inch; 
    $y = $nn-0.2;
    ...
    open(PLT, $plt3);
    if ($tensor[1] eq "MomentTensor" and ($meca[18] ne "0.00" or $meca[21] ne "0.00")) {
       printf PLT "0 0 0 @tensor[9,4,7,6] %f %f 23\n",-$tensor[8],-$tensor[5];
    } else {
       print PLT "0 0 0 @meca[5,6,7] 5\n";
    }
    #$x = 2;
    #foreach (@others) {
       #split;
